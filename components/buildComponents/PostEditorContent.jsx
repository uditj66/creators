...
// Import quill styles only on the client side
...
const quillConfig = {
  modules: {
    toolbar: {
      ...
      // Handler will be set dynamically in the component
...
const PostEditorContent = ({ form, setQuillRef, onImageUpload }) => {
  ...
  const [isGenerating, setIsGenerating] = useState(false); // Consistent naming
  const [isImproving, setIsImproving] = useState(false);

  // Helper to check for content (fixed the '<p><br><p>' issue)
  const hasContent =
    watchedValues.content && watchedValues.content !== "<p><br></p>";
  const hasTitle = watchedValues.title?.trim();

  // Dynamically set the image upload handler for the Quill toolbar
  ...
  const handleAI = async (type, improvementType = null) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        // Handle error: check for message property, otherwise use the whole error
        const errorMessage =
          result.error?.message || result.error || "Unknown AI error";
  ...
  return (
    <>
        ...
        <div className="space-y-5">
          ...
          {watchedValues.featuredImage ? (
            <div className="relative group">
              {/* Using standard img tag for simplicity with dynamic/external images */}
              <img
          ...
          <div>
            {!hasContent ? (
              <Button
                ...
                variant="outline"
                size="sm"
                // Using Codebase 1's opacity for consistency
          ...
          {(isGenerating || isImproving) && (
            // Using 100% width from Codebase 2
      ...
      {/* Global CSS for Quill styling (copied from Codebase 1/2 as they were identical) */}
      <style jsx global>{`
        .ql-editor {
